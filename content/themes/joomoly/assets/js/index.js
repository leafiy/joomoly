    ...
    $(document).ready(function(){
        ...
        if(
        	...
        	if(featured_image && featured_image.length > 0) {
        		...
        		featured_image.appendTo($('.post-media'));
        	} else if(featured_video && featured_video.length > 0) {
        		featured_video.appendTo($('.post-media'));
        	}
        }
        ...
        if(
          ...
          $('.post-wrapper_helper--notloaded').each(function(i, block) {
          	blocks.push(block);
          });
          ...
          for(var i = 0; i < blocks.length; i++) {
          	add_class(blocks[i], 'post-wrapper_helper--animated', i * 200);
          }
          
          $('.post-wrapper_helper--notloaded').each(function(i, wrapper) {
            wrapper = $(wrapper);
            ...
            if(img) {
              ...
              var timer = setInterval(function() {
                ...
                if(img.complete) {
                  ...
                  img.remove();
                  var img_container = $('<div class="post-image el-transition_long" style="background-image: url(\''+src+'\')"></div>');
                  img_container.appendTo(wrapper);
                  wrapper.removeClass('post-wrapper_helper--notloaded');
                  // add class with delay
                  setTimeout(function() {
                    img_container.addClass('post-image--loaded');
                  }, 250);
                }          
              }, 500);
              // add necessary mouse events
              wrapper.mouseenter(function() {
                wrapper.addClass('post-wrapper_helper--hover');
              });

              wrapper.mouseleave(function() {
                wrapper.removeClass('post-wrapper_helper--hover');
              });
            } else {
